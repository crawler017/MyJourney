	...
	<div class="container clearfix">
		...
		<div class="main-wrap">
			...
			<div class="crumb-wrap">
                ...
                <span class="crumb-step"></span><a class="crumb-name" >客户端管理</a><span
						class="crumb-step">&gt;</span><span>接入客户端管理</span>
				</div>
			</div>
			...
			<div class="result-wrap">
					...
					<form id="customerForm" name="customerForm" action="#" method=post >
                        ...
                        <div class="result-list">
                            <a href="<?php echo site_url('Client/clientAdd'); ?>"><i class="icon-font"></i>创建客户端</a>
                        </div>
                        </div>
					...
					<div class="result-content">
						<table id=grid
							...
							<tbody>
                                ...
                                $sql = 'select * from sso_clients order by client_id asc';
                                //var_dump($sql);die;
                                // 结果集
                                ...
                                for($i=0; $i<$Num; $i++) {
                                    ...
                                    echo "<td>{$row['client_key']}</td>";
                                    echo "<td>
                                     <a href='javascript:del({$row['client_id']})'>删除</a>
                                     <a href='javascript:edit({$row['client_id']})'>修改</a>
                                     </td>";
                                    echo"<td>";
                                }
                                // 5. 释放结果集
...
<script type="text/javascript">
    function del (client_id) {
        if (confirm("确定删除吗？")){   
            window.location.href="<?php echo site_url('Client/clientDel'); ?>?client_id="+client_id;
        }
    }
</script>
</div>
...
<script type="text/javascript">
    function edit (client_id) {
        if(confirm("确定修改此客户端吗？")){
         window.location.href="<?php echo site_url('Client/clientChange'); ?>" + '/' + client_id;
        }
    }
</script>
</body>
